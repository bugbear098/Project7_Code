...
"""
Combine 1H 3CD features with 1m base data using OPEN-STATE alignment and strict,
ID-aware early-purge detection.

- Builds 1H 'open-state' by shifting close-state features back one bar.
- Aligns 1H open-state to 1m (backward).
- Strict purge: bull if 1m.high > anchor_high; bear if 1m.low < anchor_low.
- Counts early purge exactly once per draw ID, only if minute != 59.
- Zeroes *_eff columns from purge minute forward until ID changes/clears.
- total_3CD_events = unique 1H draw IDs first seen on the 1m grid.

This version uses Python 3.9-compatible type hints.
"""
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "combined_1m_1H_openstate_eff.csv")
...
def build_open_state(df: pd.DataFrame) -> pd.DataFrame:
    """
    Build 1H 'open-state' by shifting close-state columns back one bar.
    Returns: datetime, draw_open, anchor_high_open, anchor_low_open,
    ...
    if missing:
        raise ValueError(f"1H file missing required columns: {missing}")
...
def id_key(ts: Optional[pd.Timestamp]) -> Optional[int]:
    """Return stable integer key (ns since epoch) for a Timestamp, or None for NaT/None."""
...
def main():
    ...
    # Load 1H close-state (bar-start) and build open-state
    ...
    # Rename aligned columns with 1H_ prefix
    combined.rename(columns={
        "draw_open":            "1H_open_draw",
        "anchor_high_open":     "1H_open_anchor_high",
        "anchor_low_open":      "1H_open_anchor_low",
        "range_open":           "1H_open_range",
        "ratio_open":           "1H_open_ratio",
        "draw_id_open":         "1H_open_draw_id",
    }, inplace=True)

    # Types
    combined["1H_open_draw"]    = pd.to_numeric(combined["1H_open_draw"], errors="coerce").fillna(0).astype("int8")
    combined["1H_open_range"]   = pd.to_numeric(combined["1H_open_range"], errors="coerce").fillna(0.0)
    combined["1H_open_ratio"]   = pd.to_numeric(combined["1H_open_ratio"], errors="coerce").fillna(0.0)
    combined["1H_open_anchor_high"] = pd.to_numeric(combined["1H_open_anchor_high"], errors="coerce")
    combined["1H_open_anchor_low"]  = pd.to_numeric(combined["1H_open_anchor_low"],  errors="coerce")
    combined["1H_open_draw_id"]     = pd.to_datetime(combined["1H_open_draw_id"], errors="coerce")

    # Effective copies (edited on purge); raw open-state retained
    combined["1H_third_candle_draw_eff"]          = combined["1H_open_draw"].astype("int8")
    combined["1H_third_candle_range_eff"]         = combined["1H_open_range"]
    combined["1H_third_candle_body_to_range_eff"] = combined["1H_open_ratio"]
    combined["1H_anchor_high_eff"]                = combined["1H_open_anchor_high"]
    combined["1H_anchor_low_eff"]                 = combined["1H_open_anchor_low"]
    combined["1H_draw_started_at_eff"]            = combined["1H_open_draw_id"]
    ...
    sign_series = combined["1H_open_draw"].to_numpy()
    id_series   = pd.to_datetime(combined["1H_open_draw_id"], errors="coerce")
    ah_series   = combined["1H_open_anchor_high"].to_numpy()
    al_series   = combined["1H_open_anchor_low"].to_numpy()
    ...
    def zero_eff_span(start_idx: int, end_idx_excl: int):
        """Zero effective 1H features on [start_idx, end_idx_excl)."""
    ...
    while i < n:
        ...
        # Count total unique 3CD events (first time we see a non-NaT ID on 1m grid)
        ...
        if not in_draw:
            # Activate only if:
            #  - open-state indicates active draw (raw_sign != 0)
            #  - valid draw ID
            #  - ID not yet killed by an earlier purge
            ...
            # Natural end without purge if sign goes 0, ID clears, or ID changes
            ...
            if purged:
                # Only count once per ID and only if minute != 59
                if (raw_id_k not in killed_ids) and (minute_of_hour[i] != 59):
                    early_purge_count += 1

                # Mark this ID as killed so it can't reactivate later
    ...
    print(f"[INFO] total_3CD_events (unique 1H draw IDs on 1m grid) = {total_3CD_events}")
    print(f"[INFO] early_purge_count (strict, minute != 59, ID-aware) = {early_purge_count}")
